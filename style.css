/* ============================================================
   CVC GROUP v16 — Design System
   Manrope (display/headings) + DM Sans (body/UI)
   All serifs removed — premium modern sans-serif pairing
   ============================================================ */
...
/* Remove italic-accent serif class — replaced with weight/color treatment */
...
.logo-mark {
  font-family: var(--f-display); font-size: 28px; color: var(--cream);
  letter-spacing: .06em; font-weight: 200;
}
...
.nav-links a {
  ...
  color: var(--cream-2); position: relative; padding: 6px 0;
...
.nav-cta {
  ...
  padding: 11px 24px; font-size: 11px; letter-spacing: .24em; text-transform: uppercase;
...
.btn {
  ...
  font-weight: 500; border: 1px solid transparent; transition: all .35s var(--t);
  cursor: pointer;
}
...
.hero-tags {
  ...
  color: var(--cream-2); margin-bottom: 30px;
}
.hero-tags span { color: var(--muted); }
.hero-title { margin-bottom: 32px; max-width: 760px; }
.hero-sub {
  max-width: 480px; color: var(--muted-2); font-size: 15px; line-height: 1.7;
  margin-bottom: 42px;
}
...
.hero-dot {
  ...
  position: relative; padding-right: 28px; cursor: pointer;
  transition: color .3s var(--t); font-weight: 500;
}
.hero-dot::after {
...
.hero-arrows {
  position: absolute; right: 32px; bottom: 60px; display: flex; gap: 12px; z-index: 3;
}
.hero-arrow {
...
/* SERVICE CARD TITLES: BLACK in default state */
.tri-item h3 {
  margin-bottom: 12px;
  color: var(--ink);          /* ← black default */
  transition: color .35s var(--t);
  font-weight: 500;
}
.tri-item:hover h3 { color: var(--ink); } /* stays readable on hover */
.tri-item p {
  color: #5a5e64; font-size: 14px; line-height: 1.7; margin-bottom: 22px;
  max-width: 280px; margin-left: auto; margin-right: auto;
}
/* Refined hover: gold underline on link */
.tri-item .link-cta {
  opacity: .7; transition: opacity .35s var(--t), gap .3s var(--t);
}
...
.proj-carousel-header .section-head {
  text-align: left; margin-bottom: 0;
}
.proj-carousel-header .section-head h2 { margin: 0; }

.carousel-controls {
  display: flex; align-items: center; gap: 12px; flex-shrink: 0; padding-bottom: 4px;
}
.carousel-btn {
  ...
  transition: all .35s var(--t); cursor: pointer; flex-shrink: 0;
}
...
/* Viewport — clips track, applies edge fade */
.carousel-viewport {
...
.proj-card-title {
  font-family: var(--f-display); font-size: clamp(18px, 2.2vw, 26px);
  font-weight: 300; color: var(--cream);
  letter-spacing: -.01em; margin-bottom: 10px; line-height: 1.2;
}
.proj-card-cat {
...
.proj-card-link {
  ...
  color: var(--gold-lt); border-bottom: 1px solid rgba(217,181,107,.35);
...
.carousel-progress-track {
  height: 1px; background: var(--line-strong); position: relative; overflow: hidden;
}
...
/* ============ APPROACH — with hover descriptions ============ */
...
/* Fixed height — no layout shift on hover */
.approach-item {
  padding: 0; text-align: center;
  border-right: 1px solid var(--line);
  position: relative; overflow: hidden;
  height: 220px; /* fixed — prevents any layout shift */
  cursor: default;
}
...
.approach-item:hover .approach-front {
...
.approach-item:hover .approach-back {
...
.approach-back h4 { font-size: 11px; letter-spacing: .16em; color: var(--gold-lt); margin-bottom: 2px; }
.approach-desc {
  font-size: 12px; color: var(--muted-2); line-height: 1.6; text-align: center;
}
/* Touch: show stacked, no overlay trick */
@media (hover: none) {
  .approach-item { height: auto; }
  .approach-front { position: static; opacity: 1 !important; transform: none !important; padding: 28px 16px 14px; }
  .approach-back { position: static; opacity: 1 !important; transform: none !important; background: transparent; padding: 0 16px 28px; }
}

/* ============ STATS ============ */
.stats {
  background: var(--cream); color: var(--ink); padding: 90px 0;
  position: relative; overflow: hidden;
}
.stats-bg {
  position: absolute; right: -40px; top: 50%; transform: translateY(-50%);
  width: 380px; opacity: .16; pointer-events: none;
}
.stats-grid {
...
.stat-num {
  font-family: var(--f-display); font-size: 64px; font-weight: 200;
  color: var(--ink); line-height: 1; margin-bottom: 12px; letter-spacing: -.03em;
}
...
.brands-rows { display: flex; flex-direction: column; gap: 0; }
...
.brand-row-2 { display: none; } /* single row only */
...
.brand-logo {
  ...
  transition: opacity .3s var(--t); cursor: default;
}
...
/* ============ CTA CINEMATIC ============ */
/* Replaces the old 3-column cta-block with full-bleed image + text overlay */
.cta-cinematic {
  position: relative; overflow: hidden;
...
.cta-cinematic:hover .cta-cinematic-bg { transform: scale(1); }
/* Dark gradient overlay — bottom-weighted */
.cta-cinematic-overlay {
  position: absolute; inset: 0; z-index: 1;
  background:
    linear-gradient(180deg, rgba(10,12,15,.2) 0%, rgba(10,12,15,.65) 50%, rgba(10,12,15,.94) 100%),
    linear-gradient(90deg,  rgba(10,12,15,.5) 0%, transparent 60%);
}
.cta-cinematic-inner {
  position: relative; z-index: 2;
  width: 100%; max-width: var(--container);
  margin: 0 auto; padding: 0 32px 88px;
...
.cta-cinematic-eyebrow::before {
  content: ""; width: 18px; height: 1px; background: var(--gold); display: block; flex-shrink: 0;
}
.cta-cinematic-headline {
  font-family: var(--f-display); font-weight: 200;
  font-size: clamp(38px, 6vw, 82px); line-height: 1.0;
  letter-spacing: -.03em; color: var(--cream);
  max-width: 820px; margin-bottom: 22px;
}
.cta-cinematic-headline .italic-accent { color: var(--gold-lt); }
.cta-cinematic-sub {
  font-size: 16px; color: rgba(245,241,234,.7);
  line-height: 1.65; max-width: 460px; margin-bottom: 42px;
}
.cta-cinematic-actions { display: flex; gap: 16px; flex-wrap: wrap; align-items: center; }
...
.footer-links a {
  font-size: 11px; letter-spacing: .22em; text-transform: uppercase;
  color: var(--muted-2); transition: color .25s var(--t);
}
...
.footer-social a {
  ...
  display: flex; align-items: center; justify-content: center; color: var(--cream-2);
...
.page-hero {
  ...
  background: var(--bg-dark); overflow: hidden; border-bottom: 1px solid var(--line);
}
...
.tl-year { font-family: var(--f-display); font-size: 30px; color: var(--gold); margin-bottom: 8px; line-height: 1; font-weight: 200; }
.tl-item h4 { font-size: 18px; margin-bottom: 10px; font-family: var(--f-body); font-weight: 600; letter-spacing: .02em; }
.tl-item p { color: var(--muted-2); font-size: 14px; line-height: 1.7; max-width: 480px; }
...
.filter-btn {
  padding: 11px 22px; font-size: 11px; letter-spacing: .24em; text-transform: uppercase;
  border: 1px solid var(--line-strong); color: var(--cream-2);
  transition: all .3s var(--t); background: transparent; font-weight: 500;
}
.filter-btn:hover { border-color: var(--gold); color: var(--gold); }
.filter-btn.active { background: var(--gold); border-color: var(--gold); color: var(--ink); }
.projects-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
/* Reuse proj-card for project page grid */
.proj-card {
  position: relative; aspect-ratio: 3/4; overflow: hidden; cursor: pointer; background: #222;
  display: block;
}
...
.proj-card::after {
  content: ""; position: absolute; inset: 0;
  background: linear-gradient(to top, rgba(10,12,15,.95) 0%, rgba(10,12,15,.5) 38%, transparent 70%);
}
...
.contact-card {
  border: 1px solid var(--line); padding: 28px 30px; margin-bottom: 14px;
  display: flex; gap: 20px; align-items: flex-start; transition: border-color .3s var(--t);
}
.contact-card:hover { border-color: var(--gold); }
.contact-card-icon {
  width: 44px; height: 44px; flex-shrink: 0; color: var(--gold);
...
.contact-form h3 { margin-bottom: 28px; font-size: 24px; font-weight: 300; }

.form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px; }
.form-input, .form-select, .form-textarea {
  width: 100%; background: var(--surface); border: 1px solid var(--line);
  padding: 14px 18px; color: var(--cream); font-family: var(--f-body);
  font-size: 13px; transition: border-color .25s var(--t);
}
...
@media (max-width: 980px) {
  ...
  .nav-links.open {
    ...
    padding: 32px; gap: 24px; border-top: 1px solid var(--line);
  }
  .tri, .approach-grid { grid-template-columns: repeat(2, 1fr); }
  ...
  .split, .two-col, .contact-grid { grid-template-columns: 1fr; gap: 0; }
  .two-col { gap: 40px; padding: 0 0 40px; }
  .split-text, .cta-cinematic-inner { padding: 60px 32px; }
  ...
  .values, .cap-grid, .projects-grid, .showcase-grid { grid-template-columns: repeat(2, 1fr); }
  .proj-card-c { width: calc(100vw - 80px) !important; }
  .carousel-track { padding: 0 22px; gap: 12px; }
}
@media (max-width: 560px) {
  .tri, .values, .cap-grid, .projects-grid, .showcase-grid { grid-template-columns: 1fr; }
  ...
  .cta-cinematic { min-height: 520px; }
}

/* ============================================================
   v16 ADDENDUM — Items 8–12
   ============================================================ */

/* ============ 8. HOMEPAGE CONTACT — Premium split layout ============ */
.home-contact {
  display: grid;
  grid-template-columns: 1fr 1fr;
  min-height: 680px;
}
/* Left col — bg image with gradient overlay */
.home-contact-left {
  position: relative;
  padding: 90px 72px;
...
.home-contact-left-bg {
  position: absolute; inset: 0; z-index: 0;
  background-size: cover; background-position: center;
}
.home-contact-left-bg::after {
  content: ""; position: absolute; inset: 0;
  background: linear-gradient(
    180deg,
    rgba(10,12,15,.25) 0%,
    rgba(10,12,15,.55) 50%,
    rgba(10,12,15,.88) 100%
  );
}
/* All children above bg */
.home-contact-left > * { position: relative; z-index: 1; }
.home-contact-left .eyebrow { margin-bottom: 18px; color: var(--gold-lt); }
.home-contact-left h2 {
  margin-bottom: 18px; font-size: clamp(28px,3.4vw,50px);
  color: #fff; line-height: 1.1;
}
.home-contact-left > p {
  color: rgba(245,241,234,.72); font-size: 15px; line-height: 1.7;
  max-width: 36ch; margin-bottom: 32px;
}
/* Remove old info block styles — simplified left */
...
.home-contact-form .form-select {
  appearance: none; cursor: pointer; color: var(--muted);
  background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='%239aa0a8' fill='none'/%3E%3C/svg%3E");
  background-repeat: no-repeat; background-position: right 18px center;
...
@media (max-width: 900px) {
  ...
  .home-contact-left,
  .home-contact-right { padding: 60px 32px; border-right: none; border-bottom: 1px solid var(--line); }
  .home-contact-form .form-row { grid-template-columns: 1fr; }
}

/* ============ 9. TIMELINE — alternating, dynamic ============ */
.timeline-alt {
  position: relative;
  padding: 20px 0 60px;
}
...
.timeline-alt::before {
  ...
  width: 1px; background: linear-gradient(180deg, transparent, var(--line-strong) 8%, var(--line-strong) 92%, transparent);
...
/* Left-side items: content left, empty right */
...
/* Right-side items: empty left, content right */
...
.tl-marker-dot {
  ...
  transition: background .4s var(--t), transform .4s var(--t);
  flex-shrink: 0;
}
.tl-alt-item:hover .tl-marker-dot {
...
.tl-year-badge {
  ...
  border: 1px solid var(--gold-soft); padding: 5px 14px;
  margin-bottom: 16px;
}
.tl-content h4 {
  font-family: var(--f-display); font-size: clamp(18px, 1.8vw, 24px);
  font-weight: 300; letter-spacing: -.01em; color: var(--cream);
  margin-bottom: 12px; line-height: 1.25;
}
.tl-content p {
  font-size: 14px; color: var(--muted-2); line-height: 1.7; max-width: 36ch;
}
.tl-alt-item.left .tl-content p  { margin-left: auto; }
.tl-alt-item.right .tl-content p { margin-right: auto; }

/* Visual block — image or stat card */
.tl-visual { }
.tl-img-wrap {
  position: relative; overflow: hidden;
  aspect-ratio: 16/10;
...
.tl-stat-num {
  font-family: var(--f-display); font-size: clamp(42px,5vw,64px);
  font-weight: 200; line-height: 1; letter-spacing: -.03em; color: var(--gold);
}
.tl-stat-label {
  font-size: 10px; letter-spacing: .28em; text-transform: uppercase;
...
@media (max-width: 860px) {
  ...
  .tl-alt-item + .tl-alt-item { margin-top: 48px; }
  .tl-marker { position: absolute; left: 15px; top: 24px; flex-direction: row; }
  .tl-alt-item { position: relative; }
  .tl-content,
  .tl-alt-item.left .tl-content,
  .tl-alt-item.right .tl-content {
    padding: 0 !important; text-align: left !important;
  }
  .tl-visual,
  .tl-alt-item.left .tl-visual,
  .tl-alt-item.right .tl-visual {
    padding: 20px 0 0 !important;
    display: none; /* hide images on mobile for clarity */
  }
  .tl-content p { margin-left: 0 !important; margin-right: 0 !important; max-width: none; }
  .tl-year-badge { margin-bottom: 12px; }
}

/* ============ 10. SECONDARY PAGE CTA — Rebalanced split ============ */
/* Replaces old .cta-block on inner pages */
.page-cta {
  background: var(--bg-dark);
  display: grid;
  grid-template-columns: 1fr 1fr;
...
.page-cta-img::after {
  content: ""; position: absolute; inset: 0;
  background: linear-gradient(90deg, rgba(10,12,15,.3), rgba(10,12,15,.0) 50%),
              linear-gradient(180deg, transparent 50%, rgba(10,12,15,.5) 100%);
}
.page-cta-content {
  padding: 80px 72px;
...
@media (max-width: 860px) {
  ...
  .page-cta-img { min-height: 280px; position: relative; }
  .page-cta-img img { position: absolute; }
  .page-cta-content { padding: 56px 32px; border-left: none; border-top: 1px solid var(--line); }
}

/* ============ 11. "About Us" section (reference image) ============ */
.about-feature {
...
.about-feature-headline {
  ...
  font-size: clamp(24px,2.6vw,38px); line-height: 1.2;
  letter-spacing: -.02em; color: var(--ink); margin-bottom: 32px;
  max-width: 22ch; /* prevents awkward second-line breaks */
}
.about-feature-headline em { font-style: normal; color: var(--gold); }

.about-feature-stat-row {
  display: flex; align-items: flex-end; gap: 24px; margin-bottom: 24px;
}
.about-feature-big-num {
  font-family: var(--f-display); font-size: clamp(80px,10vw,140px);
  font-weight: 200; line-height: .85; letter-spacing: -.05em;
  color: var(--ink);
}
.about-feature-stat-label {
  font-family: var(--f-display); font-size: 13px; font-weight: 300;
  color: var(--muted); line-height: 1.4; padding-bottom: 14px;
  max-width: 10ch;
}
...
/* Body text + cta on right half */
.about-feature-body {
  font-size: 14px; color: #5a5e64; line-height: 1.75;
  max-width: 42ch; margin-bottom: 28px; margin-top: 16px;
}
.about-feature-right {
...
@media (max-width: 900px) {
  ...
  .about-feature-big-num { font-size: clamp(80px,20vw,140px); }
}

/* ============ MISC CONSISTENCY FIXES ============ */
/* Ensure old .cta-block no longer used on inner pages */
/* (replaced by .page-cta — CSS kept for backward compat) */
.cta-block {
  display: grid; grid-template-columns: .85fr 1.15fr;
  background: var(--bg-dark);
}
.cta-block .cta-text {
  padding: 80px 64px; display: flex; flex-direction: column; justify-content: center; gap: 20px;
}
.cta-block .cta-text h2 { margin-bottom: 0; }
.cta-block .cta-text p { color: var(--muted-2); font-size: 15px; line-height: 1.75; max-width: 34ch; }
.cta-block .cta-img {
  position: relative; overflow: hidden; min-height: 460px;
  grid-column: unset !important; /* remove old span:2 */
}
.cta-block .cta-img img {
  position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover;
  transition: transform 1s var(--t);
}
.cta-block:hover .cta-img img { transform: scale(1.04); }
.cta-block .cta-img::after {
  content: ""; position: absolute; inset: 0;
  background: linear-gradient(90deg, rgba(10,12,15,.45) 0%, transparent 40%);
}
@media (max-width: 860px) {
  .cta-block { grid-template-columns: 1fr; }
  .cta-block .cta-img { min-height: 280px; }
  .cta-block .cta-text { padding: 56px 32px; }
}
...
@media (max-width: 420px) {
  .brands-tile-grid { grid-template-columns: 1fr; }
}

/* ── Approach responsive fixes for touch ── */
@media (hover: none) {
  .approach-item { height: auto; border-bottom: 1px solid var(--line); }
  .approach-front {
    position: static !important; opacity: 1 !important; transform: none !important;
    padding: 28px 16px 10px; pointer-events: auto !important;
  }
  .approach-back {
    position: static !important; opacity: 1 !important; transform: none !important;
    background: transparent !important; padding: 0 16px 28px;
